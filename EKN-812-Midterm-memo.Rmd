...
We want to establish the expected price of gas that would make the purchase of the Prius, that is more expensive but consumes less, less costly than the Polo, that is less expensive but consumes more.
...
These give us the (inverse) supply curves of the $i^{th}$ firm. Notice that the suply of y is perfectly elastic: firms are indifferent about the quantity they sell (they just sell at cost $p_a$), but they will not supply any $y_i$ at $p_y<p_a$, and they will try to sell $y_i=+\infty$ at $p_y>p_a$. 

Notice also that if we wanted to get the supply of $x_i$ by firm i, we would set $p_x=\alpha_ix_i^{\theta^{-1}}$ and solve: we get $x_i^*(p_x)=(\frac{p_x}{\alpha_i})^\theta$. Notice finally that $\theta$ is the elasticity of supply. 
...
Then we get: $v^*=p_v^{-\epsilon}=(p_x+\beta^{-1}p_y)^{-\epsilon}$,
and so $x_j^*=v_j^*=(p_x+\beta^{-1}p_y)^{-\epsilon}$

and $\beta y_j^*=v_j^* \implies y_j^*=\beta^{-1}(p_x+\beta^{-1}p_y)^{-\epsilon}$.
...
The only tricky part is finding the equilibirum price of meals, $p^*_x$. But using (5) and (6) and substituting $p_y^*=p_a$ from (8), we get: 
\begin{align*}
Np_x^\theta &=M(p_x+\beta^{-1}p_a)^{-\epsilon} \\
\iff p_x^\theta(p_x+\beta^{-1}p_a)^{\epsilon}&=\frac{M}{N}
\end{align*}
...
\begin{align*}
Np_x^\theta = M(p_x+\beta^{-1}p_a)^{-\epsilon} & \iff &  \log N +\theta \log p_x = \log M - \epsilon \log (p_x+\beta^{-1}p_a) 
\end{align*}

Let us say $LHS(p_x)=\log N+ \theta \log p_x$

and $RHS(p_x) = \log M-\epsilon \log (p_x+\beta^{-1}p_a)$. Graphically, the two functions look as follows:
...
On the other hand as $p_x \rightarrow 0$, then the $RHS \rightarrow logM-\epsilon log(\beta^{-1} p_a)>-\infty$, and as $p_x \rightarrow \infty$, then $RHS \rightarrow -\infty$. This means that there will exist at least one equilibrium. 
...
\noindent(h) The price of alcohol is just $p_a$ (the input price facing restaurants). And, if we look back at the diagram we used to show existence and uniqueness of the equilibrium, we notice that $p_a$ only enters the expression from the RHS (the log of aggregate demand). In particular, when $p_a$ increases, theis shifts down the RHS curve, lowering both price and quantity. 

The interpretation is straightofrward: consumers like to consume meals and drinks in fixed proportions. So if drinks become more expensive, this pushes down the demand for meals. 

Algebraically, we could implicitly differentiate the market-clearing condition to get: 
...
Now, in the long run, N adjusts such that $\pi_i=0$, so this gives that the long run price $\bar{p}_x=[f(1+\theta)]^\frac{1}{1+\theta}$

Now, because equilibrium prices satisfy $p_x^2+\beta^{-1}p_ap_x-(\frac{M}{N}^{1/\theta})=0$, we get, imposing $p_x=\bar{p}_x=[f(1+\theta)]^\frac{1}{1+\theta}$, that $$\bar{N}=\frac{M}{\Big[[f(1+\theta)]^{(2/(1+\theta))}+\beta^{-1}p_af(1+\theta)\Big]^\theta}$$
